...
export default function PostRentPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Rental Property Listed Successfully!</h2>
          <p className="text-gray-600 mb-4">
            Your rental property has been submitted and is now live. Redirecting...
          </p>
        </div>
      </div>
    )
  ...
  const rentQuota = quota?.rent || { currentListings: 0, maxListings: 2, remainingSlots: 0, canCreateListing: false }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="relative overflow-hidden bg-slate-900 text-white py-8 md:py-12">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl md:text-4xl font-bold mb-2">List Your Property for Rent</h1>
          <p className="text-base md:text-xl text-slate-300">
            {quota?.role === 'AGENT' ? 'Create a professional rental listing' : 'Rent your property directly to tenants'}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Quota Info Card */}
        {quota && (
          <div className={`mb-6 p-4 rounded-lg ${rentQuota.canCreateListing ? 'bg-cyan-50 border border-cyan-200' : 'bg-red-50 border border-red-200'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-start sm:items-center gap-3">
                {rentQuota.canCreateListing ? (
                ...
                <div>
                  <p className="font-medium text-gray-900 text-sm md:text-base">
                    Rental Listing Quota: {rentQuota.currentListings} / {rentQuota.maxListings}
                  </p>
                  <p className="text-xs md:text-sm text-gray-600">
                    {rentQuota.canCreateListing
                      ? `You can create ${rentQuota.remainingSlots} more rental listing${rentQuota.remainingSlots > 1 ? 's' : ''}`
                      : 'Mark a rental property as INACTIVE to free up a slot'}
                  </p>
                </div>
              </div>
        ...
        {quota && !rentQuota.canCreateListing && (
          <div className="bg-red-100 border border-red-300 p-6 rounded-lg mb-6 text-center">
            ...
            <h3 className="text-lg font-bold text-red-800 mb-2">Rental Listing Limit Reached</h3>
            <p className="text-red-700 mb-4">
              You have reached your maximum of {rentQuota.maxListings} active rental listings.
              {quota.role !== 'AGENT' && ' Consider upgrading to Agent for 10 rental listings.'}
            </p>
            {quota.role !== 'AGENT' && (
              <Link
                href="/profile"
                ...
                <div>
                  ...
                  <div className="mb-2 p-3 bg-cyan-50 border border-cyan-200 rounded-lg">
                    <p className="text-sm text-cyan-700">
                      ...
                      <br />
                      <span className="text-xs text-cyan-700">Example: "3 Bedroom House for Rent in DHA Phase 5 Lahore"</span>
                    </p>
                  </div>
                  <input
                    type="text"
                    ...
                    placeholder="e.g., Beautiful 3 Bedroom House for Rent in DHA Phase 5"
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {formData.price && (
                    <p className="mt-2 text-sm text-gray-500">
                      ≈ Rs. {parseFloat(formData.price).toLocaleString()} / month
                    </p>
                  )}
                </div>
                ...
                <div className="bg-cyan-50 p-4 rounded-lg">
                  <p className="text-sm text-cyan-700">
                    <strong>Tip:</strong> Properties priced competitively based on market rates typically rent 20% faster.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={6}
                    placeholder="Describe your rental property in detail. Include key features, nearby amenities, and what makes it special..."
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500"
                    required
                  />
